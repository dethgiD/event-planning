...
const getAllTaskUpdatesByTaskIdAndEventId = async (req, res) => {
  try {
    const taskUpdates = await prisma.taskUpdate.findMany({
      where: {
    ...
    if (taskUpdates.length > 0) {
      ...
      res.status(404).json({ error: 'No task updates found for this task and event' });
    }
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
module.exports = {
  ...
  getAllTaskUpdatesByTaskIdAndEventId
};

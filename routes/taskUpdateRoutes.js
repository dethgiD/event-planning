...
router.post('/', taskUpdateValidationRules, validateRequest, async (req, res) => {
  ...
  try {
    const taskUpdate = await TaskUpdate.create({ taskId, updateText });
...
router.get('/:id', 
  ...
  async (req, res) => {
    try {
      const taskUpdate = await TaskUpdate.findByPk(req.params.id);
      if (taskUpdate) {
        res.json(taskUpdate);
      } else {
...
router.put('/:id', 
  ...
  async (req, res) => {
    ...
    try {
      const taskUpdate = await TaskUpdate.findByPk(req.params.id);
      if (taskUpdate) {
        await taskUpdate.update({ updateText });
        res.json(taskUpdate);
      } else {
        res.status(404).json({ error: 'Task Update not found' });
      }
